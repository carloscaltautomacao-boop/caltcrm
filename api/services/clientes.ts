...
// Busca o cliente pelo telefone ou cria um novo (lead chegando).
export async function obterOuCriarPorTelefone(telefone: string): Promise<Cliente> {
  const { rows } = await query<Cliente>('SELECT * FROM clientes WHERE telefone = $1', [telefone]);
  if (rows[0]) return rows[0];
  const novo = await query<Cliente>(
    `INSERT INTO clientes (telefone, etapa, origem) VALUES ($1, 'novo', 'trafego') RETURNING *`,
    [telefone],
  );
  return novo.rows[0]!;
}
